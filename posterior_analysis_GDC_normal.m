% Posterior analysis for GDC estimates
...
function write_file = posterior_analysis_GDC_normal(draws, count, write_path, snap_dt, nof_z_vec)
% Inputs:
...
post_draws = [draws.a   draws.b    draws.c   draws.d   draws.e  draws.f];
for j = 1:m
...
para_name = [name_a; name_b; {'c'}; {'d'}; {'e'}; {'f'}];
for j = 1:m
...
MH_para_name = [name_absum; name_ap; {'cdsum'}; {'cp'}; {'efsum'}; {'ep'}]; %name of the parameters that did MH steps
count_mat = [count.absum   count.ap  count.cdsum  count.cp  count.efsum  count.ep]; %matrix of the counts of MH acceptance
