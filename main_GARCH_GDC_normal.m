...
%
% GDC bottom: x_{ij,t} = (1 - c - d) * x_ij + c * u_{i,t-1} * u_{j,t-1} + d * x_{ij,t-1},
...
gdc.efsum_q0 = 1;
gdc.ep_p0 = 2; %prior: ep ~ Beta(ep_p0, ep_q0)
gdc.ep_q0 = 2;
...
df.absum = [0.1  0.5]; %[0.3  1]; %increase => lower acceptance rate
...
df.ep = 4; %2; %increase => lower acceptance rate
...
if nof_pred == 0 %full-sample estimation
    [draws,count] = MCMC_GARCH_GDC_normal(y, z, z_coef, garch, gdc, df, nof_burnin, nof_draws); %MCMC estimation; show progress every 1000 draws.
    save([write_path, 'results_GDC.mat'], 'draws', 'count'); %use "results = load('results_DCC.mat') to load data.
    ...
    for pred_j = 1:nof_pred
        ...
        [draws,count] = MCMC_GARCH_GDC_normal(Yt_est, Zt_est, z_coef, garch, gdc, df, nof_burnin, nof_draws); %MCMC estimation; show progress every 1000 draws.
...
[hist_count_post3, bin_loc_post3] = hist(draws.efsum, 100);
[hist_count_post4, bin_loc_post4] = hist(draws.ep, 100);

[hist_count_prior1, bin_loc_prior1] = hist(betarnd(gdc.cdsum_p0, gdc.cdsum_q0, nof_draws, 1), 100);
[hist_count_prior2, bin_loc_prior2] = hist(betarnd(gdc.cp_p0, gdc.cp_q0, nof_draws, 1), 100);
[hist_count_prior3, bin_loc_prior3] = hist(betarnd(gdc.efsum_p0, gdc.efsum_q0, nof_draws, 1), 100);
[hist_count_prior4, bin_loc_prior4] = hist(betarnd(gdc.ep_p0, gdc.ep_q0, nof_draws, 1), 100);
...
subplot(2,2,3);
plot(bin_loc_prior3, hist_count_prior3/nof_draws);
hold on;
plot(bin_loc_post3, hist_count_post3/nof_draws, 'r');
hold off;
legend('prior e+f', 'posterior e+f');

subplot(2,2,4);
plot(bin_loc_prior4, hist_count_prior4/nof_draws);
hold on;
plot(bin_loc_post4, hist_count_post4/nof_draws, 'r');
hold off;
legend('prior e/(e+f)', 'posterior e/(e+f)');
saveas(gcf, [write_path, 'compare_prior_post_gdc.fig']);
