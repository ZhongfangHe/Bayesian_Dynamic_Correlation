% Estimate bottom-integrated GDC model with normal innovation
...
df.cp = 0.01; %increase => lower acceptance rate
...
if nof_pred == 0 %full-sample estimation
    [draws,count] = MCMC_GARCH_BIGDC_normal_approx(y, z, z_coef, garch, gdc, df, nof_burnin, nof_draws); %MCMC estimation; show progress every 1000 draws.
    save([write_path, 'results_BIGDC_approx.mat'], 'draws', 'count'); %use "results = load('results_DCC.mat') to load data.
    ...
    for pred_j = 1:nof_pred
        ...
        [draws,count] = MCMC_GARCH_BIGDC_normal_approx(Yt_est, Zt_est, z_coef, garch, gdc, df, nof_burnin, nof_draws); %MCMC estimation; show progress every 1000 draws.
...
saveas(gcf, [write_path, 'compare_prior_post_bigdc.fig']);
