% Estimate top-integrated GDC model with normal innovation
...
write_path = 'C:\Users\534474366\Documents\Research\DCC GARCH\Mar2019\simulated_data\results_TIGDC\'; %path to write results; should already exist
...
if nof_pred == 0 %full-sample estimation
    [draws,count] = MCMC_GARCH_TIGDC_normal(y, z, z_coef, garch, gdc, df, nof_burnin, nof_draws); %MCMC estimation; show progress every 1000 draws.
    save([write_path, 'results_TIGDC.mat'], 'draws', 'count'); %use "results = load('results_DCC.mat') to load data.
    ...
    for pred_j = 1:nof_pred
        ...
        [draws,count] = MCMC_GARCH_TIGDC_normal(Yt_est, Zt_est, z_coef, garch, gdc, df, nof_burnin, nof_draws); %MCMC estimation; show progress every 1000 draws.
...
hold off;
legend('prior e', 'posterior e');
saveas(gcf, [write_path, 'compare_prior_post_tigdc.fig']);
