...
function write_file_pred = info_pred_GDC_normal(logp_vec, pred_para_est, pred_mh, snap_dt, write_path) %store est info to check if anything goes wrong
...
para_name = [{'log_pred_density'}; name_absum; name_ap; {'cdsum'}; {'cp'}; {'efsum'}; {'ep'};...
    name_ar_absum; name_ar_ap; {'AccptRate_cdsum'}; {'AccptRate_cp'}; {'AccptRate_efsum'}; {'AccptRate_ep'}];

write_file_pred = [write_path, 'Forecast.csv'];
if isempty(snap_dt) == 0 %first column is snap date
    ind_date = 1;
    write_mat = cell(nof_pred+1, 6+4*m+4);
    write_mat{1,1} = 'Date';
    write_mat(1,2:(6+4*m+4)) = para_name';
    nobs_raw = length(snap_dt);
    write_mat(2:(nof_pred+1),1) = snap_dt((nobs_raw-nof_pred+1):nobs_raw);
    write_mat(2:(nof_pred+1),2:(6+4*m+4)) = num2cell([logp_vec  pred_para_est  pred_mh]);     
else %first column is numbers, not snap dates
    ind_date = 0;
    write_mat = cell(nof_pred+1, 5+4*m+4);
