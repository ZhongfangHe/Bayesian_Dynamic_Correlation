...
%
% cDCC bottom: x_{ij,t} = (1 - c - d) * x_ij + c * sqrt(x_{ii,t-1} * x_{jj,t-1}) * u_{i,t-1} * u_{j,t-1} + d * x_{ij,t-1},
...
write_path = 'C:\Users\534474366\Documents\Research\DCC GARCH\Mar2019\simulated_data\results_cDCC\'; %path to write results; should already exist
...
read_info.read_file = [read_path, 'Simulated_Data_cDCC.xlsx']; 
...
% Specify the prior for cDCC parameters
...
nof_draws = 5000; %number of effective draws for posterior analysis
...
if nof_pred == 0 %full-sample estimation
    [draws,count] = MCMC_GARCH_cDCC_normal(y, z, z_coef, garch, dcc, df, nof_burnin, nof_draws); %MCMC estimation; show progress every 1000 draws.
    save([write_path, 'results_cDCC.mat'], 'draws', 'count'); %use "results = load('results_DCC.mat') to load data.
    write_file = posterior_analysis_cDCC_normal(draws, count, write_path, snap_dt, nof_z_vec); %write the results
    ...
    for pred_j = 1:nof_pred
        ...
        [draws,count] = MCMC_GARCH_cDCC_normal(Yt_est, Zt_est, z_coef, garch, dcc, df, nof_burnin, nof_draws); %MCMC estimation; show progress every 1000 draws.

        logp_vec(pred_j) = loglike_pred_cDCC_normal(Yt, Zt, draws); %compute log predictive density 
        ...
        write_file_pred = info_pred_cDCC_normal(logp_vec, pred_para_est, pred_mh, snap_dt, write_path); %write the results 
